...
<html>
  <head>
		 ...
		 <link href="../css/template.css" rel="stylesheet">
		  ...
		  /* The ExpediaIcons.ttf file has nice round icons, but the plane icon is tilted at an angle. ExpediaStraightPlanes.ttf gives me a straight plane for the moving animations. */
		  ...
		  <div class="container-fluid">
		    ...
		    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
		      <ul class="nav navbar-nav">
		        <li><a href="../index.php">Releases</a></li>
		        <li><a href="org.php">Tech Org Chart</a></li>
		        <li class="active"><a href="bookings.php">Booking Map</a></li>
		        <li class="dropdown">
		      ...
		      <form class="navbar-form navbar-right" role="navigation">
			      <ul class="nav navbar-nav navbar-right">
								  <div id="titleBox">
								    <h1>&nbsp;</h1>
								    <span id="about" class="Apip" title="about" onclick="showAbout();"></a>
								  </div>
								  <div id="versionBox" onclick="showAbout();">
								    <p id="aboutText"></p>
								    <p id="contactText"></p>
								    <p id="versionText"></p>
								  </div>
  ...
  // the indexIncrements are used to keep the indexes generated by drawPing and drawPing2 distinct, since they run in parallel.
  ...
  var ApipColor = "rgba(255, 210, 0, 0.6)"; // yellow
  var EMainColor = "rgba(255,10,0,0.6)"; // red
  var MobileWebColor = "rgba(140,0,200,1)"; // purple
  var E3ColorOrigin = "rgba(255, 210, 0, 0.6)"; // faded yellow

  var refreshRate = 62; // In seconds, determines how long it will take to animate all of the current date.
  ...
  var longitude = function(d) { 
                     LOB = [d.LOB]+"";
                     if(LOB=='hot'||LOB=='car') {
                      return projection([d.LONGITUDE, d.LATITUDE])[0]-15; // For non-flights,
                     } else if (LOB=='flt'||LOB.match(/pkg/)=='pkg') {
                      return projection([d.ORIG_LONGITUDE, d.ORIG_LATITUDE])[0]-15; // For flights, we want to draw the ORIGIN first
                     }
                  }; // the -20 and -5 are added to try to line the ping up on the center of the coordinates
             
  var latitude= function(d) { 
                     LOB = [d.LOB]+"";
                    if(LOB=='hot'||LOB=='car') {
                      return projection([d.LONGITUDE, d.LATITUDE])[1]+5; // For non-flights,
                     } else if (LOB=='flt'||LOB.match(/pkg/)=='pkg') {
                      return projection([d.ORIG_LONGITUDE, d.ORIG_LATITUDE])[1]+5; // For flights, we want to draw the ORIGIN first
                     }
                  }; // the -20 and -5 are added to try to line the ping up on the center of the coordinates
  ...
  var destLongitude = function(d) {
                    ...
                    if (LOB=='flt'||LOB.match(/pkg/)=='pkg') {
                      return projection([d.DEST_LONGITUDE, d.DEST_LATITUDE])[0]; // Only flights and packages have a DESTINATION
                     }
                  };
  ...
  var destLatitude= function(d) { 
                    LOB = [d.LOB]+"";
                    if (LOB=='flt'||LOB.match(/pkg/)=='pkg') {
                      return projection([d.DEST_LONGITUDE, d.DEST_LATITUDE])[1]; // Only flights and packages have a DESTINATION
                      }
                    };
  ...
  // Create the element group to mark individual bookings:
  var bookings = svg.append('svg:g')
                  .attr('id', 'bookings');
  // Create an element group for the first thread of drawings (makes cleanup easier)
  var draw1 = bookings.append('g')
                .attr('id', 'bookings-'+indexIncrement1);
  // Create an element group for the second thread of drawings (makes cleanup easier)
  ...
  d3.json('../json/world-countries.json', function(collection) { 
          ...
          features.append('svg:title')
            //.text( function(d) { return d.properties.name; }) //this causes the countries to show their names when hovered over
            ;
        });
...
-->
  ...
  <script>
    ...
    d3.select("#about")
      ...
      .style("font-size","15px")
    d3.select("#aboutText")
      .text("The map shows all EAN bookings that go through the API. Real time booking data is sourced from an event subscriber, which is fed by OMS. Lat/long info sourced from Tailwind. Created by Tim Annan, based on the EWE BookingMap created by Richard Houchin and Lance Shankles. Please contact tannan@expedia.com with suggestions.")
/*
    d3.select("#contactText")
      .text("Legend: ( API Partners) ( API E*) ( Chameleon) ( Chameleon Mobile)")
      .style("font-family","ExpediaIcons2")
      .style("font-size","15px")
*/
    drawPing();
